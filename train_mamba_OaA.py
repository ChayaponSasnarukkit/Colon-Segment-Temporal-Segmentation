...
def to_binary_labels(labels, target_class_idx=5, ignore_index=-100):
    """
    Converts multi-class labels to binary 1-vs-All.
    Class 1 = Target Class (Splenic Flexure)
    Class 0 = Everything Else
    """
    # 1 if target class, 0 otherwise
    ...
    # Restore the ignore_index (-100)
    binary_labels = torch.where(
        labels == ignore_index, 
        torch.tensor(ignore_index, device=labels.device), 
        binary_labels
    )
...
def train_one_epoch(model, dataloader, optimizer, device):
    ...
    for step, (vision_embeddings, _, labels, reset_mask, _, worker_id) in enumerate(tqdm(dataloader)):
        ...
        # Convert to 1-vs-All (Splenic Flexure is index 5)
        labels = to_binary_labels(labels, target_class_idx=5)
...
def main():
    ...
    # 2. Toggle for your experiment
    USE_WEIGHTED_LOSS = True  # Set to False for your unweighted experiment
    ...
    for epoch in range(epochs):
        ...
        for idx, f1 in enumerate(val_f1_per_class):
            ...
            class_name = BINARY_IDX_TO_CLASS.get(idx, f"Class_{idx}")
