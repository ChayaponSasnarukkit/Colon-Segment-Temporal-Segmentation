...
def detach_states(states):
    ...
    if isinstance(states, dict):
        ...
        if 'tcn_caches' in states and states['tcn_caches'] is not None:
            ...
            # Check if it's the new 2-stage dictionary format
            ...
            # Fallback for the old 1-stage list format
...
def train_one_epoch(model, dataloader, optimizer, device, accumulation_steps=16, 
    ...
    """for step, batch in enumerate(tqdm(dataloader)):
        ...
        if with_future:
            ...
            ce_loss = 0.15*loss_wo + 1.0*loss_w + 0.3*loss_future
...
def main():
    ...
    cfg_virtual_batch_size = hparams.get("virtual_batch_size", 12)
    ...
    checkpoint_path = os.path.join(cfg_pretrain_dir, "neww3best_mamba_model.pth") if cfg_pretrain_dir else ""
    ...
    save_dir = f"/project/pv927002-dcpcce/4mid_smoothing_dampw_realcolon_fold{FOLD}"
    os.makedirs(save_dir, exist_ok=True)
    best_model_path = os.path.join(save_dir, "re4_32drop5expansion4stages2batch12lr03.pth")
