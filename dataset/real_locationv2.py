...
NUM_CLASSES = len(LABEL_MAP)
import os.path as osp
class RealColonStreamingNoJitterDataset(IterableDataset):
    def __init__(self, 
                 video_root, 
                 batch_size_per_worker, 
...
NUM_CLASSES = len(LABEL_MAP)
from collections import Counter
class RealColonInferStreamingDataset(IterableDataset):
    def __init__(self, 
                 video_root, 
                 batch_size_per_worker, 
                 split_dir=None,
    ...
    def __iter__(self):
        ...
        while any(s is not None for s in active_streams):
            ...
            # --- NEW: Arrays to track video IDs and frame indices for this batch ---
            batch_vid_id, batch_frame_idx = [], []
            ...
            for i in range(self.batch_size):
                ...
                if stream is None:
                    ...
                    # --- NEW: Handle empty slots by adding a placeholder "PAD" and padding frames with -1 ---
                    batch_vid_id.append("PAD")
                    batch_frame_idx.append(torch.full((self.chunk_size,), -1, dtype=torch.long))
                    continue
                ...
                # --- NEW: Convert frame indices to a 1D tensor to yield them ---
                curr_frame_idx_tensor = torch.tensor(curr_indices, dtype=torch.long)
                ...
                if actual_sampled_len < self.chunk_size:
                    ...
                    # --- NEW: Ensure frame indices are padded identically to the inputs (-1 for padded frames) ---
                    frame_idx_pad = torch.full((pad_len,), -1, dtype=torch.long)
                    curr_frame_idx_tensor = torch.cat([curr_frame_idx_tensor, frame_idx_pad], dim=0)
                ...
                # --- NEW: Add the active stream's vid_id and padded frame tensor to our batch lists ---
                batch_vid_id.append(stream['vid_id'])
                batch_frame_idx.append(curr_frame_idx_tensor)
            ...
            # --- NEW: Prep vid_id and frame_idx for yielding ---
            final_vid_id = tuple(batch_vid_id)       # Tuple of strings: (vid_1, vid_2, ...)
            final_frame_idx = torch.stack(batch_frame_idx) # Tensor of shape [batch_size, chunk_size]
            
            # --- NEW: Added the two new outputs at the end of the yield ---
            yield final_curr, final_ctx, final_lbl, final_future_lbl, final_mask, final_ctx_mask, worker_id, final_vid_id, final_frame_idx