...
from dataset.real_locationv2 import MedicalStreamingDataset, CLASS_MAP
...
def main():
    ...
    # --- 5. Pre-Training Validation Check ---
    print("\n-------------------------------------------------------")
    print("Running validation check to verify model loading...")
    val_loss, val_acc, val_f1_macro, val_f1_per_class = validate(
        full_model, val_loader, device, transition_penalty_loss
    )
    print(f"Loaded Model Check -> Val Loss: {val_loss:.4f} | Acc: {val_acc:.4f} | Macro F1: {val_f1_macro:.4f}")
    print("-------------------------------------------------------\n")
