...
def train_one_epoch(model, dataloader, optimizer, device, accumulation_steps=4, 
    ...
    for step, batch in enumerate(tqdm(dataloader)):
        ...
        # Output(loss=loss, logits=logits, next_states=new_states, hidden_states=hidden_states)
        output = model(
        ...
        smooth_loss = compute_temporal_smoothing_loss(logits, labels)
        jump_loss = transition_penalty_loss(logits, labels)
...
def validate(model, dataloader, device, transition_penalty_loss, 
    ...
    for step, batch in enumerate(tqdm(dataloader, desc="Validating")):
        ...
        preds = torch.argmax(logits, dim=-1) # [B, M]
...
def main():
    ...
    loss_fn = nn.CrossEntropyLoss(weight=f1_based_weights.to(device), ignore_index=-100)
    ...
    for epoch in range(epochs):
        ...
        train_loss = train_one_epoch(model, train_loader, optimizer, device, lambda_smooth=0.5, lambda_jump=0.0)
        
        # 2. Validate (Now receiving metrics)
        val_loss, val_acc, val_f1_macro, val_f1_per_class = validate(model, val_loader, device, transition_penalty_loss)
