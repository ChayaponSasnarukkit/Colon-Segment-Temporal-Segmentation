...
#     print("Warning: f1_based_weights not found. Defaulting to unweighted CE.")
f1_based_weights = None
# --- 1. 50Salads Class Map ---
CLASS_MAP = {
    "cut_tomato": 0,
...
class TransitionPenaltyLoss(nn.Module):
    def __init__(self, num_classes=19, ignore_index=-100):
        super().__init__()
...
def validate(model, dataloader, device, transition_penalty_loss, 
    ...
    # Updated to evaluate across all 19 classes of 50salads
...
def cache_predictions(model, dataloader, device, save_path):
    ...
    for step, batch in enumerate(tqdm(dataloader, desc="Caching Predictions")):
        ...
        _, _, _, tcn_logits, next_states = model(
            vision_embeddings=vision_embeddings,
            contexts=valid_contexts, pass_states=current_states, labels=labels
        )

        probs = F.softmax(tcn_logits, dim=-1)
        B, L = labels.shape
...
def main():
    ...
    best_model_path = os.path.join(cfg_save_dir, "best_model.pth")
    cache_save_path = os.path.join(cfg_save_dir, f"predictions_fold{cfg_fold}.npz")

    # Initialize 50Salads Dataset
    train_dataset = FiftySaladsStreamingDataset(
    ...
    transition_penalty_loss = TransitionPenaltyLoss(num_classes=num_action_classes).to(device)

    print("\n--- Validation Check (Pre-Train) ---")
    # Using -100 as standard background ignore index, although 50salads users sometimes add 'action_start' (17) and 'action_end' (18) to bg_classes for overlap eval
    val_loss, val_acc, val_f1_macro, val_f1_per_class, _, _, _ = validate(full_model, val_loader, device, transition_penalty_loss, bg_class=[-100])
    ...
    for epoch in range(cfg_epochs):
        ...
        val_loss, val_acc, val_f1_macro, val_f1_per_class, val_edit, val_f1_overlaps, val_b_mae = validate(
            full_model, val_loader, device, transition_penalty_loss, bg_class=[-100]
        )
